...
export const potReducer = (state, action) => {
  switch (action.typ) {
    ...
    case POT_ACTIONS.DELETE_POT:
      const filteredPots = state.pots.filter(
    ...
    case POT_ACTIONS.EDIT_POT:
      const editPot = state.pots.map((pot) =>
        pot.id === action.payload.id ? { ...pot, ...action.payload } : pot
      );
      return {
        ...state,
        pots: editPot,
...
export const PotProvider = ({ children }) => {
  const [state, dispatch] = useReducer(potReducer, {
    ...initialState,
    pots: pots.pots,
    totalMoney: pots.totalSaved,
  });

  return (
    <PotContext.Provider value={{ state, dispatch }}>
      {children}
    </PotContext.Provider>
  );
};
